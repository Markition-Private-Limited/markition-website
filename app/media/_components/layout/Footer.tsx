export default function Footer() {
  return (
    <footer>
      ...
      {/* ── Main body ── */}
      <div className="media-footer-main" style={{ background: "#020a1c", padding: "72px 0 56px" }}>
        <div
      ...
      {/* ── Bottom bar ── */}
      <div className="media-footer-bottom" style={{ background: "#010610", padding: "16px 32px" }}>
        <div
          className="media-footer-bottom-inner"
          style={{
            maxWidth: 1200,
            margin: "0 auto",
            display: "flex",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {links.map(({ label, href }) => (
        <a
          key={label}
          href={href}
          style={{ fontSize: 13.5, color: "#8aa4c8", textDecoration: "none" }}
        >
          {label}
        </a>
      ))}
...
function SocialLink({ href, label, icon }: { href: string; label: string; icon: React.ReactNode }) {
  return (
    <a href={href} aria-label={label} target="_blank" rel="noopener noreferrer"
      style={{ display: "flex", alignItems: "center", justifyContent: "center", color: "#0096b4" }}>
      {icon}
    </a>
  );
}
